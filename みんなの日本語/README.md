...
Notes taken as we are going through the book in class.

> [!CAUTION]
> These notes are written for me in the way I want them // the way that makes sense to me.
> There will almost certainly be some mistakes as I am shit at Japanese.

Where there are extra notes for any lesson point, the lesson point will be linked to the additional notes in the `Lesson Point` column.

## Lesson 1

| Lesson Point  | Example | Short Summary                                                                            |
| ------------- | ------- | ---------------------------------------------------------------------------------------- |
| は            | `わたしは　Aです` (I am A) | Particle to mark subject of a sentence, In this case `わたし`/`I`    |
...
| X-も (not any X)            | `何も 食べません` (I do not eat anything) | `どこにも` (Nowhere), `なにも` (Nothing) `だれも` (Nobody). `も` is suffixed to question words to mean none of question. This is used along with negative sentences.  |
...
| Lesson Point                 | Example | Short Summary                                                                                      |
| ---------------------------- | ------- | -------------------------------------------------------------------------------------------------- |
| [を](../Grammar/particles/を.md)　     | `おさけを　のみますか` (do you drink alcohol?) | Marks the direct object of a verb.                     |
...
| [もらいました　(receiving)](../Grammar/もらう.md) | `せんせいに　ほんを　もらいました`　(I received a book from sensei) | Used to say you received something. |
...
| [い-Adjectives And な-Adjectives (Negating)](../Grammar/conjugating/Adjectives-い-And-な-conjugations.md) | `n/a` | The way you create the negative for of an Adjective depends on the adjective type. The exceptions have their own rules as well. Please see full article. |
| よ (New Info)                          | `わたしの　くにの　8月は　あまり　あつくないです　よ` (In my country its not very hot in August.) | The `よ` particle can be placed at the end of a sentence when the speaker expects the information they are providing to be new to the listener. In the case of the example the speaker does not expect the listener knows about the weather in their country |
...
| たい (want)                 | `ビールを　のみたいです` (I want to drink beer)   | Conjugates a verb to indicate wanting to do |
| なにか (Anything?)          | `なにか　食べたいです` (I want to eat anything)　  | not sure on details.... `tbd` |
| [に (Purpose)](../Grammar/particles/に.md) | `渋谷へ　お酒を　のみに　いきました` (I went to Shibuya to drink). `よこはま　へ　コンサート　に　いきました`　(I went to Yokohoma for a concert). | `に` marks the reason you go somewhere. In this case the `まし` is removed from the Verb. So the form is `<Place>へ<thing>を<Verb>に<いきまし>`. |
...
| て-Form (な-Adjectives)             | `親切で　きれい　です` ((Subject) is Kind and Pretty)        　　            | Used to link multiple adjectives together to create more descriptive sentences. `で` is appended to the `な-adjective` to create the `て-Form`. |
